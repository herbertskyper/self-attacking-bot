...
namespace globalPlanningCpp{
    ...
    std::priority_queue<std::pair<double,int>> openlist;
    std::vector<int> openlist_1;
    class GlobalPlanner:public rclcpp::Node{ 
        ...
        explicit GlobalPlanner(const rclcpp::NodeOptions & options = rclcpp::NodeOptions())
            ...
            point.resize(node_tot+1);v.resize(node_tot);d.resize(node_tot);
            for(int i=0;i<node_tot;++i)
        ...
        void nav_global_server_start(){RCLCPP_INFO(this->get_logger(),"loading...");rclcpp::spin(node_global_server);rclcpp::shutdown();}
        ...
        double caldistance(Eigen::Vector2d a,Eigen::Vector2d b)
        {
            return (a-b).norm();
        }
        bool globalPlanning_reload(Eigen::Vector2d end_point){
            //RCLCPP_INFO(this->get_logger(),"reloading...1\n\n\n");

            //A* 改寫 

            for(int i=0;i<node_tot;++i) d[i]=0;
            auto able_id = nav_pos_find(end_point);
            if(able_id.empty()) return 0;
            


            while(!ver[node_tot].empty() && rclcpp::ok()) ver[node_tot].pop_back();
            
            point[node_tot][0]=end_point[0];
            point[node_tot][1]=end_point[1];
            
            for(int i=0;i<(int)able_id.size();i++)
            ver[node_tot].push_back(able_id[i]);
            int closelist[100]={0};
            information_node node_status[100];
            int node_current;
                while(!openlist.empty() && rclcpp::ok()) openlist.pop();
                while(!openlist_1.empty() && rclcpp::ok()) openlist_1.pop_back();
                memset(closelist,0,sizeof(int)*100);
                openlist.push(std::make_pair(0,node_tot));

                while(!openlist.empty() && rclcpp::ok())
                {
                    node_current=openlist.top().second;
                    closelist[node_current]=1;
                    //auto pos_node=find(openlist_1.begin(),openlist_1.end(),node_current);
                    openlist.pop();
                    //openlist_1.erase(pos_node);
                    int flag=0;
                    for(auto y:ver[node_current])
                    {
                        if(closelist[y]==0)
                        {
                        for(auto i=openlist_1.begin();i!=openlist_1.end();i++)
                        {
                            if(*i==y)
                            {
                                flag=1;
                        ...
                        if(!flag)
                        {
                            node_status[y].pri_id=node_current;
                            node_status[y].g=caldistance(point[y],point[node_current]);
                            node_status[y].h=0;//caldistance(point[y],point[i])
                            node_status[y].f=node_status[y].g+node_status[y].h;
                            openlist.push(std::make_pair(-node_status[y].f,y));
                            openlist_1.push_back(y);
                        }
                        else
                        {
                            double tmpg = caldistance(point[y],point[node_current]);
                            if (tmpg<node_status[y].g)
                            {
                                node_status[y].pri_id=node_current;
                                node_status[y].g = tmpg;
                                node_status[y].f = tmpg + node_status[y].h;
                            }
                        }
                        flag=0;
                        }
                ...
                for(int i=0;i<node_tot;i++)
                {
                        ...
                        do
                        {
                            d[i]+=(point[y1]-point[node_status[y1].pri_id]).norm();
                            y1=node_status[y1].pri_id;
                        }while(!(point[y1][0]==end_point[0] && point[y1][1]==end_point[1]));
                }
        ...
        int global_direction_once(Eigen::Vector2d start_point){
            ...
            for(int j=0,lim = able_id.size();j<lim;++j){
                int i = able_id[j];
                if((start_dis > d[i] && (point[i]-start_point).norm() > 0.1)){
