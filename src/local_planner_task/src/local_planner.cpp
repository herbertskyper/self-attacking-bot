...
namespace localPlanningCpp{
    ...
    std::vector<std::vector<double>> stone_queue;
    std::vector<std::vector<std::vector<double>>> wall_queue;
    std::vector<std::vector<double>> obsBoxQueue;
    static bool encounter_wall=0;
    static Eigen::Vector2d target_point_pri;
    class LocalPlanner:public rclcpp::Node{
        ...
        explicit LocalPlanner(const rclcpp::NodeOptions & options)
            ...
            const auto stone_queue = toml::find<std::vector<std::vector<double>>>(toml_file,"STONE_OF_QUEUE");
            const auto wall_queue = toml::find<std::vector<std::vector<std::vector<double>>>>(toml_file,"WALL_OF_QUEUE");
            for(auto i:wall_queue)
            {
                    for(int k=0;k<100;k++)
                    {
                        ...
                        temp.clear();
                        temp.push_back(i[0][0]+(i[1][0]-i[0][0])/100);temp.push_back(i[0][1]+(i[1][1]-i[0][1])/100);
                        //obsBoxQueue.push_back(temp);
                    }
            }
            for(auto i:stone_queue)
            obsBoxQueue.push_back(i);
        }
        ...
        void local_direction(Eigen::Vector2d end_point){
            ...
            for(auto i:obsBoxQueue)
            {

                if(pow(i[0]-target_point[0],2)+pow(i[1]-target_point[1],2)<0.7*0.7)
                {
                    //RCLCPP_INFO(this->get_logger(),"%lf\n",pow(i[0]-target_point[0],2)+pow(i[1]-target_point[1],2));
                    encounter_wall=1;
                    target_point_pri[0]=target_point[0];  target_point_pri[1]=target_point[1];
                    break;
                }
                    
                else if(encounter_wall==1 && pow(target_point[0]-target_point_pri[0],2)+pow(target_point[1]-target_point_pri[1],2)>=0.2*0.2)
                    encounter_wall=0;
            }
            if(encounter_wall==1)
            {
